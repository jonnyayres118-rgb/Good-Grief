...
function HowItWorks({ onStart }) {
  const cards = [["01", "Make your plan", "Draft the whole plan for free, in any order and at your own pace."], ["02", "Secure it", "Choose Annual or Lifetime to save it privately across devices."], ["03", "Share what matters", "Invite up to three trusted people and choose exactly what each person can see."]];
...
function SignupModal({ plan, mode, onClose, onComplete, onDraft = onClose }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><div className="modal signup-modal" role="dialog" aria-modal="true" aria-labelledby="signup-title" onMouseDown={event => event.stopPropagation()}><button className="modal-close" onClick={onClose} aria-label="Close"><X /></button><p className="eyebrow eyebrow--pink">{signin ? "WELCOME BACK" : "SECURE MY PLAN"}</p><h2 id="signup-title">{signin ? "Pick up your plan." : "Create your account."}</h2><p>{signin ? "Your wishes and access list will be right where you left them." : `Your draft is saved on this device. Create your account, then choose ${plan === "lifetime" ? "£75 lifetime" : "£29 annual"} to secure it across devices.`}</p><form onSubmit={submit}>{!signin && <label>Your name<input required value={form.name} onChange={event => setForm({ ...form, name: event.target.value })} placeholder="Alex Smith" /></label>}<label>Email address<input required type="email" value={form.email} onChange={event => setForm({ ...form, email: event.target.value })} placeholder="alex@example.com" /></label><label>Password<input required minLength="8" type="password" value={form.password} onChange={event => setForm({ ...form, password: event.target.value })} placeholder="At least 8 characters" /></label>{!signin && <label className="check-label"><input type="checkbox" checked={form.consent} onChange={event => setForm({ ...form, consent: event.target.checked })} /><span>Send me occasional, useful Good Grief emails. Optional.</span></label>}{error && <p className="form-error">{error}</p>}<button className="button button--sun button--full" disabled={busy}>{busy ? "One moment…" : signin ? "Sign in" : "Create account"} <ArrowRight /></button></form>{!signin && <button className="text-link signup-draft-link" onClick={onDraft}>Keep exploring without an account <ArrowRight /></button>}<small>{isLiveBackend ? "Account protected by Supabase Auth." : "Local preview mode. Production never bypasses account or payment checks."}</small></div></div>;
}
...
function AccessModal({ onClose }) {
  const [summary, setSummary] = useState({ peopleWithAccess: [], plansSharedWithMe: [], inviteLimit: 3, activeInviteCount: 0, invitesRemaining: 3 });
...
function ShareModal({ answers, membership, onClose, onPay, onOpenAccess }) {
  ...
  const submit = async (event) => {
    event.preventDefault(); setBusy(true); setError("");
    try { if (!permissions.length) throw new Error("Choose at least one area to share."); const result = await createInvite(email, answers, permissions); setInviteUrl(result.inviteUrl); setDelivery(result.delivery || null); }
    catch (err) { setError(err.message); }
    finally { setBusy(false); }
  };
  const copy = async () => { await navigator.clipboard?.writeText(inviteUrl); setCopied(true); };
  if (!membership?.paid) return <div className="modal-backdrop"><div className="modal share-modal share-locked"><button className="modal-close" onClick={onClose} aria-label="Close"><X /></button><div className="payment-icon"><LockKey size={28} weight="fill" /></div><p className="eyebrow eyebrow--pink">Paid membership required</p><h2>Secure your plan. Unlock sharing.</h2><p>Your draft stays free on this device. Secure it with Annual or Lifetime membership to save across devices and invite three trusted people.</p><button className="button button--sun button--full" onClick={() => onPay("annual")}>Unlock for £29/year <ArrowRight /></button><button className="button button--ghost button--full" onClick={() => onPay("lifetime")}>Choose £75 lifetime</button></div></div>;
  return <div className="modal-backdrop"><div className="modal share-modal"><button className="modal-close" onClick={onClose} aria-label="Close"><X /></button><p className="eyebrow eyebrow--pink">Invite someone you trust</p><h2>{inviteUrl ? delivery?.sent ? "Invitation sent." : "Invitation ready." : "Who should see what?"}</h2>{!inviteUrl ? <><p>You can invite up to three trusted people. Enter the email address they must use, then choose exactly what they can see.</p><form onSubmit={submit}><label>Their email address<input required type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="someone@example.com" /></label><fieldset className="permission-picker"><legend>They can see</legend>{productAreas.map(area => <label key={area.key}><input type="checkbox" checked={permissions.includes(area.key)} onChange={() => togglePermission(area.key)} /><span>{area.title}</span></label>)}</fieldset>{error && <p className="form-error">{error}</p>}<button className="button button--sun button--full" disabled={busy}>{busy ? "Creating invitation…" : "Create private invitation"} <UserPlus /></button></form><div className="invite-benefit"><Tag weight="fill" /><div><b>A useful thank-you.</b><span>After accepting, they receive 50% off the first year of their own Annual plan.</span></div></div></> : <><p>{delivery?.sent ? <>We emailed <b>{email}</b>. </> : <>Copy this link and send it privately to <b>{email}</b>. </>}The plan stays hidden until they sign in, and they will see only the areas you selected.</p><label>Invitation link<div className="share-link"><span>{inviteUrl}</span><button onClick={copy}>{copied ? <Check /> : <Copy />}</button></div></label><button className="button button--ink button--full" onClick={copy}>{copied ? "Copied" : "Copy invitation"}<Copy /></button></>}<button className="text-link access-link" onClick={onOpenAccess}><UsersThree /> See everyone with access</button></div></div>;
}
...
function Planner({ onHome }) {
  ...
  useEffect(() => { (async () => { try { const user = await currentUser(); setOwnerName(user?.user_metadata?.full_name || user?.name || ""); const entitlement = user ? await getEntitlement().catch(() => ({ paid: false, paymentStatus: "unpaid" })) : { paid: false, paymentStatus: "unpaid" }; setMembership(entitlement); const stored = await loadCloudPlan({ paid: entitlement.paid }); setAnswers(normalizePlan(stored.answers || {})); setUpdatedAt(stored.updatedAt || null); setStatus(stored.storage === "secure" ? "Saved securely" : "Saved on this device"); } catch { setStatus("Saved on this device"); } finally { setHydrated(true); } })(); }, []);
  ...
  useEffect(() => { if (!hydrated) return; setStatus("Saving…"); const timer = setTimeout(async () => { try { const result = await persistPlan(answers, { paid: membership.paid }); const savedAt = result.updatedAt || new Date().toISOString(); setUpdatedAt(savedAt); setStatus(result.storage === "secure" ? "Saved securely" : "Saved on this device"); } catch { setStatus("Saved on this device"); } }, 650); return () => clearTimeout(timer); }, [answers, hydrated, membership.paid]);
...
function SharedPlan({ token }) {
  ...
  return <main className="shared-page"><header><Brand /><span><LockKey weight="fill" /> Read-only Good Grief plan</span></header><section>{membership?.referralDiscountAvailable && !membership?.paid && <div className="referral-banner"><Tag weight="fill" /><div><b>Your invitation benefit</b><span>50% off your first year: £14.50 today, then £29/year.</span></div><button className="button button--sun" onClick={() => window.location.assign("/planner")}>Start my free draft <ArrowRight /></button></div>}<p className="eyebrow eyebrow--pink">{shared.ownerName} trusts you with this</p><h1>The things they want you to know.</h1><div className="shared-grid">{planSteps.filter(step => answerFor(sharedAnswers, step)).map(step => <article key={step.key}><p>{step.areaTitle} · {step.title}</p><h2>{step.prompt}</h2><div>{answerFor(sharedAnswers, step)}</div></article>)}</div></section></main>;
}
