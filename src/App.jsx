...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowSquareOut, CaretRight, Check, CheckCircle, Copy, CreditCard, DownloadSimple, EnvelopeSimple, Eye, FileText, Heart, House, Link, List, LockKey, MusicNotes, PencilSimple, ShieldCheck, Sparkle, Tag, Trash, UserPlus, UsersThree, WarningCircle, X } from "@phosphor-icons/react";
...
import { exportPlanPdf } from "./lib/exportPlanPdf.js";
import { productAreas, planSteps } from "./content/product.js";
import { calculateProgress, containsSensitiveSecret, normalizePlan } from "./domain/plan.js";

const answerFor = (answers, step) => answers?.[step.areaKey]?.[step.field] || "";
const updateAnswer = (answers, step, value) => ({ ...answers, [step.areaKey]: { ...(answers[step.areaKey] || {}), [step.field]: value } });
...
function ShareModal({ answers, membership, onClose, onPay, onOpenAccess }) {
  ...
  if (!membership?.paid) return <div className="modal-backdrop"><div className="modal share-modal share-locked"><button className="modal-close" onClick={onClose} aria-label="Close"><X /></button><div className="payment-icon"><LockKey size={28} weight="fill" /></div><p className="eyebrow eyebrow--pink">PAID MEMBERSHIP REQUIRED</p><h2>Finish your plan. Unlock sharing.</h2><p>You can keep writing for free. Invitations only become available after Stripe confirms an Annual or Lifetime payment.</p><button className="button button--sun button--full" onClick={() => onPay("annual")}>Unlock for £29/year <ArrowRight /></button><button className="button button--ghost button--full" onClick={() => onPay("lifetime")}>Choose £75 lifetime</button></div></div>;
...
function PlanHome({ answers, membership, ownerName, updatedAt, onEdit, onReview, onAccess, onMembership }) {
  const progress = calculateProgress(answers);
  const nextStep = Math.max(0, planSteps.findIndex(step => !String(answerFor(answers, step)).trim()));
  const date = updatedAt ? new Intl.DateTimeFormat("en-GB", { day: "numeric", month: "short", year: "numeric" }).format(new Date(updatedAt)) : "Today";
  return <section className="planner-home"><div className="planner-home-hero"><div><p className="eyebrow eyebrow--pink">{ownerName ? `Hello, ${ownerName}` : "My Good Grief"}</p><h1>You’re {progress.percent}% sorted.</h1><p>Look at you being organised. Pick up one area or leave it for another day.</p><div className="planner-home-actions"><button className="button button--sun" onClick={() => onEdit(nextStep)}>Continue my plan <ArrowRight /></button><button className="button button--ghost" onClick={onReview}>Review everything <Eye /></button></div></div><div className="completion-disc"><b>{progress.percent}%</b><span>{progress.completed} of {progress.total}<br />details added</span></div></div><div className="area-dashboard">{productAreas.map((area) => { const steps = planSteps.filter(step => step.areaKey === area.key); const done = steps.filter(step => String(answerFor(answers, step)).trim()).length; const first = planSteps.findIndex(step => step.areaKey === area.key && !String(answerFor(answers, step)).trim()); return <button className={`dashboard-area dashboard-area--${area.colour}`} key={area.key} onClick={() => onEdit(first < 0 ? planSteps.findIndex(step => step.areaKey === area.key) : first)}><span>{done}/{steps.length}</span><h2>{area.title}</h2><p>{area.description}</p><b>{done === steps.length ? "Nicely done" : done ? "Continue" : "Make a start"} <ArrowRight /></b></button>; })}</div><div className="planner-home-lower"><article className="readiness-card"><p className="eyebrow">Ready when it matters</p><h2>One clear source.</h2><div className="readiness-list"><p><CheckCircle weight="fill" /><span><b>Always current</b> with automatic saving.</span></p><p><CheckCircle weight="fill" /><span><b>Private by design</b> and visible only to people you choose.</span></p><p><CheckCircle weight="fill" /><span><b>No passwords needed.</b> Record where important things can safely be found.</span></p></div></article><article className="member-card"><div className="member-card-top"><span><CreditCard weight="fill" /></span><small>Membership</small></div><h2>{membership.paid ? `${membership.membershipType === "lifetime" ? "Lifetime" : "Annual"} member` : "Draft membership"}</h2><p>{membership.paid ? "PDF download and trusted sharing are unlocked." : "Draft for free. Upgrade when you’re ready to download and share."}</p><button className="text-link" onClick={onMembership}>View membership <ArrowRight /></button><footer>Last updated {date}</footer></article></div></section>;
}

function PlanReview({ answers, membership, ownerName, updatedAt, onEdit, onPay, onAccess }) {
  const [downloading, setDownloading] = useState(false);
  const [error, setError] = useState("");
  const answered = planSteps.filter(step => String(answerFor(answers, step)).trim());
  const download = async () => {
    if (!membership.paid) { onPay("annual"); return; }
    setDownloading(true); setError("");
    try { const pdfAnswers = Object.fromEntries(planSteps.map(step => [step.key, answerFor(answers, step)])); await exportPlanPdf({ answers: pdfAnswers, steps: planSteps, ownerName: ownerName || "My", updatedAt: updatedAt ? new Date(updatedAt) : new Date() }); }
    catch (err) { setError(err.message || "Unable to create your PDF."); }
    finally { setDownloading(false); }
  };
  return <section className="plan-review"><div className="review-heading"><div><p className="eyebrow eyebrow--pink">The whole plan</p><h1>Everything useful.<br /><span>All together.</span></h1><p>Read it as the people you trust will read it. Make a change, download a copy, or update access.</p></div><div className="review-actions"><button className="button button--sun" onClick={download} disabled={downloading}>{downloading ? "Designing your PDF…" : membership.paid ? "Download designed PDF" : "Unlock PDF download"}<DownloadSimple weight="bold" /></button><button className="button button--ghost" onClick={onAccess}>Manage trusted access <UsersThree /></button></div></div>{error && <p className="form-error">{error}</p>}<div className="review-document"><header><div className="brand brand--review"><span>Good</span><span>Grief</span></div><div><small>My plan</small><b>{answered.length} / {planSteps.length} added</b></div></header>{planSteps.map((item, index) => { const value = answerFor(answers, item); return <article className={`review-item ${value ? "review-item--answered" : ""}`} key={item.key}><div className="review-number">{String(index + 1).padStart(2, "0")}</div><div><p>{item.areaTitle} · {item.title}</p><h2>{item.prompt}</h2>{value ? <div>{value}</div> : <em>Not added yet.</em>}</div><button onClick={() => onEdit(index)} aria-label={`Edit ${item.title}`}><PencilSimple /></button></article>; })}<footer><span>Good Grief</span><span>Make things easier for the people you love.</span></footer></div></section>;
}
...
function Planner({ onHome }) {
  ...
  useEffect(() => { const checkout = new URLSearchParams(window.location.search).get("checkout"); refreshMembership().then(result => { if (checkout === "success") setCheckoutNotice(result.paid ? "Payment confirmed. Your PDF and trusted sharing are unlocked." : "Payment received. Stripe is confirming your membership…"); }); if (checkout === "success") { const timer = setTimeout(() => refreshMembership().then(result => result.paid && setCheckoutNotice("Payment confirmed. Your PDF and trusted sharing are unlocked.")), 2200); return () => clearTimeout(timer); } }, []);
  ...
  const current = planSteps[step]; const Icon = PencilSimple;
  const openEditor = index => { setStep(index); setView("edit"); };
  ...
  const mainContent = view === "home" ? <PlanHome answers={answers} membership={membership} ownerName={ownerName} updatedAt={updatedAt} onEdit={openEditor} onReview={() => nav("review")} onAccess={() => setAccessOpen(true)} onMembership={() => nav("membership")} /> : view === "review" ? <PlanReview answers={answers} membership={membership} ownerName={ownerName} updatedAt={updatedAt} onEdit={openEditor} onPay={openPayment} onAccess={() => setAccessOpen(true)} /> : view === "membership" ? <MembershipView membership={membership} onPay={openPayment} /> : <section className="planner-main"><div className="planner-step-icon"><Icon size={30} weight="fill" /></div><p className="eyebrow">{current.areaTitle} · {step + 1} of {planSteps.length}</p><h1>{current.prompt}</h1><p>Not sure? Skip it for now. You can change your mind whenever you like.</p><textarea value={currentValue} onChange={event => setAnswers(updateAnswer(answers, current, event.target.value))} placeholder={current.placeholder} autoFocus />{secretWarning && <p className="secret-warning"><WarningCircle weight="fill" />Please remove passwords, PINs or card details. Record where they are safely stored instead.</p>}<div className="planner-controls"><button className="button button--ghost" onClick={() => step === 0 ? nav("home") : setStep(step - 1)}><ArrowLeft /> {step === 0 ? "Overview" : "Back"}</button><button className="button button--ink" disabled={secretWarning} onClick={() => step < planSteps.length - 1 ? setStep(step + 1) : nav("review")}>{step < planSteps.length - 1 ? "Save & next" : "Review my plan"}<ArrowRight /></button></div></section>;
  return <main className="planner-shell"><header className="planner-header"><Brand /><div className="planner-save"><CheckCircle weight="fill" />{status}</div><div><button className="planner-text-button" onClick={onHome}>Exit</button><button className="button button--sun" onClick={() => setShareOpen(true)}>{membership.paid ? "Invite someone" : "Unlock full plan"} <ArrowRight /></button></div></header>{checkoutNotice && <div className={`checkout-notice ${membership.paid ? "checkout-notice--success" : ""}`}><CheckCircle weight="fill" /><span>{checkoutNotice}</span><button onClick={() => setCheckoutNotice("")} aria-label="Dismiss"><X /></button></div>}<div className="planner-layout planner-layout--hub"><aside><p className="eyebrow eyebrow--pink">My Good Grief</p><nav className="hub-nav"><button className={view === "home" ? "active" : ""} onClick={() => nav("home")}><House /><span>Overview</span></button><button className={view === "edit" ? "active" : ""} onClick={() => nav("edit")}><PencilSimple /><span>Build my plan</span></button><button className={view === "review" ? "active" : ""} onClick={() => nav("review")}><FileText /><span>Review & PDF</span></button><button onClick={() => setAccessOpen(true)}><UsersThree /><span>Who has access?</span></button><button className={view === "membership" ? "active" : ""} onClick={() => nav("membership")}><CreditCard /><span>Membership</span></button></nav><div className="hub-progress"><div><b>{completed}</b><span>of {planSteps.length}<br />details added</span></div><div className="progress"><i style={{ width: `${completed / planSteps.length * 100}%` }} /></div></div>{view === "edit" && <nav className="step-nav">{planSteps.map((item, index) => { const value = answerFor(answers, item); return <button key={item.key} className={index === step ? "active" : value ? "done" : ""} onClick={() => setStep(index)}><span>{value ? <Check weight="bold" /> : index + 1}</span>{item.title}</button>; })}</nav>}<div className={`membership-chip ${membership.paid ? "membership-chip--paid" : ""}`}><LockKey weight="fill" /><span>{membership.paid ? `${membership.membershipType} member · full plan unlocked` : "Drafting mode · PDF and sharing locked"}</span></div></aside><div className="planner-content">{mainContent}</div></div>{shareOpen && <ShareModal answers={answers} membership={membership} onClose={() => setShareOpen(false)} onPay={openPayment} onOpenAccess={() => { setShareOpen(false); setAccessOpen(true); }} />}{accessOpen && <AccessModal onClose={() => setAccessOpen(false)} />}{accountPlan && <SignupModal plan={accountPlan} mode="signup" onClose={() => setAccountPlan(null)} onDraft={() => setAccountPlan(null)} onComplete={plan => { setAccountPlan(null); setPaymentPlan(plan); }} />}{paymentPlan && <PaymentModal plan={paymentPlan} onClose={() => setPaymentPlan(null)} onPaid={() => { setPaymentPlan(null); refreshMembership(); nav("home"); }} referralDiscount={membership.referralDiscountAvailable} />}</main>;
}
